    ...
    */
    public function handle()
    {
        ...
        if(isset($character->last_fetch)){
            $now = new \DateTime();
            $last_fetch = new \DateTime($this->character->last_fetch);
            $diff = date_diff($now, $last_fetch);
            if($diff->h < 1) {
                $new_min = (60 - $diff->i);
                log::debug("Time since last update is less than 1 hour.");
            }
        }
        ...
        try {
            ...
            log::debug("We received a 5xx error from ESI, this usually means an issue on CCP's end, please try again later.");
            //5xx error, usually and issue with ESI
            return;
        } catch (\Exception $e) {
            //Everything else
            log::debug ("We failed to pull your rolls, please try again later.");
            return;
        }

        if(!in_array("Station_Manager", $roles->roles)) {
            lo::debug( "Character $character->character_name doesn't have the Station Manager Role, this is required to pull Corporation Structures. Once added please wait at least 60 minutes before trying again.");
            return ;
        } 
