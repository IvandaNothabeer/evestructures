...
class LowFuelCheck implements ShouldQueue
{
    ...
    */
    ...
    {
        $structures = Structure::with('characters')->where('fuel_expires', '!=', 'n/a')->get();
        foreach ($structures as $structure) {
            foreach ($structure->characters as $character){ 
                Log::debug("Starting FuelCheck for $structure->structure_name owned by $character->character_name on user_id $character->user_id"); 
                ...
                $fuel_notices = FuelNotice::where('user_id', $character->user_id)
                ...
                if($days_left < 1) {
                    if((isset($fuel_notices) && $fuel_notices->twentyfour_hour == FALSE ) || !isset($fuel_notices->twentyfour_hour)) {
                        ...
                        Log::debug("Sending 24hour notice (days left are $days_left) for $structure->structure_name owned by $character->character_name on user_id $character->user_id"); 
                        FuelNotice::updateOrCreate(
                            ['structure_id' => $structure->structure_id, 'user_id' => $character->user_id],
                    ...
                    if((isset($fuel_notices) && $fuel_notices->seven_day == FALSE) || !isset($fuel_notices->seven_day)) {
                        ...
                        Log::debug("Sending 7day notice (days left are $days_left) for $structure->structure_name owned by $character->character_name on user_id $character->user_id"); 
                        FuelNotice::updateOrCreate(
                            ['structure_id' => $structure->structure_id, 'user_id' => $character->user_id],
                            ['seven_day' => TRUE, 'twentyfour_hour' => FALSE, 'character_id' => $character->character_id]
                        ); 
                        $character->notify(new LowFuelDiscord($structure)); 
                    ...
                    FuelNotice::updateOrCreate(
                        ['structure_id' => $structure->structure_id, 'user_id' => $character->user_id],
                        ['seven_day' => FALSE, 'twentyfour_hour' => FALSE, 'character_id' => $character->character_id]
                    ); 
